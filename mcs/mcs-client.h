...
// An MCS client. This client is in charge of all communications with an MCS endpoint, and is capable of reliably sending/receiving GCM messages.
// NOTE: Not thread safe. This class should live on the same thread as that network requests are performed on.
class MCSClient {
	public:
		// Any change made to this enum should have corresponding change in the GetStateString(...) function.
		enum State {
		...
		typedef void(*OnMessageReceivedCallback) (const MCSMessage& message);
		// Callback when a message is sent (and receipt has been acknowledged by
		// the MCS endpoint).
		typedef void(*OnMessageSentCallback) (

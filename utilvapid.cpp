/**
  * Copied from ECEC/tools (because JWT helper code not included in the lib itself)
...
/**
  ...
  */
static inline size_t vapid_json_escape_unicode(char c, char* result) {
	result[0] = '\\';
	result[1] = 'u';
	result[2] = '0';
	result[3] = '0';
...
/**
  ...
  */
static size_t vapid_json_quoted_length(const char* str, size_t strLen) {
...
/**
  ...
  */
static char* vapid_json_quote(
	const char* str, 
	size_t strLen
) 
{
	size_t quotedLen = vapid_json_quoted_length(str, strLen);
	char* quotedStr = (char*) malloc(quotedLen + 1);
	if (!quotedStr) {
		return NULL;
	}
...
/**
  ...
  */
static char* vapid_build_signature_base(
	const char* aud, 
	size_t audLen, 
	uint32_t exp,
	const char* sub, 
	...
	int payloadLen =
	snprintf(NULL, 0, "{\"aud\":%s,\"exp\":%" PRIu32 ",\"sub\":%s}", quotedAud, exp, quotedSub);
	if (payloadLen <= 0) {
	goto end;
	}
	// Allocate an extra byte for the null terminator, which `sprintf` appends.
	payload = (char *) malloc((size_t) payloadLen + 1);
	if (!payload) {
	goto end;
	}
	if (sprintf(payload, "{\"aud\":%s,\"exp\":%" PRIu32 ",\"sub\":%s}", quotedAud, exp, quotedSub) <= 0) {
	goto end;
	}
	...
	*sigBaseLen = b64HeaderLen + b64PayloadLen + 1;
	sigBase = (char *) malloc(*sigBaseLen);
	if (!sigBase) {
	goto end;
	}
...
char* vapid_build_token(
	...
	size_t sigBaseLen;
	sigBase = vapid_build_signature_base(aud, audLen, exp, sub, subLen, &sigBaseLen);
	if (!sigBase) {
		goto error;
	}
	size_t sigLen;
	...
	token = (char *) realloc(sigBase, tokenLen + 1);
	if (!token) {
		goto error;
	}
	sigBase = NULL;
...
static EC_KEY* vapid_import_private_key(
	const char* b64PrivKey
) {
	return NULL;
}

static EC_KEY*vapid_generate_keys() {
...
static char* vapid_export_private_key(EC_KEY* key) {
	...
	char* b64PrivKey = (char *) malloc(b64PrivKeyLen + 1);
	if (!b64PrivKey) {
		return NULL;
	}
	ece_base64url_encode(rawPrivKey, ECE_WEBPUSH_PRIVATE_KEY_LENGTH,
...
static char* vapid_export_public_key(
	EC_KEY* key
) {
	uint8_t rawPubKey[ECE_WEBPUSH_PUBLIC_KEY_LENGTH];
	if (!EC_POINT_point2oct(EC_KEY_get0_group(key), EC_KEY_get0_public_key(key), POINT_CONVERSION_UNCOMPRESSED, rawPubKey, ECE_WEBPUSH_PUBLIC_KEY_LENGTH, NULL)) {
		return NULL;
	}
	size_t b64PubKeyLen =
	ece_base64url_encode(rawPubKey, ECE_WEBPUSH_PUBLIC_KEY_LENGTH,
		ECE_BASE64URL_OMIT_PADDING, NULL, 0);
	char* b64PubKey = (char *) malloc(b64PubKeyLen + 1);
	if (!b64PubKey) {
		return NULL;
	}
	ece_base64url_encode(rawPubKey, ECE_WEBPUSH_PUBLIC_KEY_LENGTH, ECE_BASE64URL_OMIT_PADDING, b64PubKey, b64PubKeyLen);
